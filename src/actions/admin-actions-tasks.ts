...
export async function createTaskAction(input: CreateTaskInput) {
  ...
  // les limites de requête. 15 Mo de fichier → ~20 Mo de base64 (~21 Mo avec overhead).
  if (input.instructions && input.instructions.length > 21_000_000) {
    return {
      success: false,
      error: "Le contenu vidéo est trop volumineux. Utilisez une vidéo de 15 Mo maximum, puis réessayez.",
    };
  }
  // Utiliser le client admin (service role) pour contourner les problèmes de session/RLS
  const adminClient = createAdminClient();
